...
from audio2face import Audio2FaceClass

from audio import AudioMixer
from webserver import Webserver
from tegrastats import Tegrastats

ttsserver = "localhost:50051"  # location of Riva server
auth = riva.client.Auth(uri=ttsserver)
...
class Chatbot(threading.Thread):
    ...
    def __init__(self, args, **kwargs):
        ...
        self.audio2face_instance = Audio2FaceClass(**vars(args))
    ...
    def send_tts(self, msg=None, end=False):
        ...
        if txt:
            ...
            resp = tts_service.synthesize(txt, voice_name=voice_name, language_code=language_code, sample_rate_hz=sample_rate_hz)
            audio = resp.audio
            audio_samples = np.frombuffer(resp.audio, dtype=data_type)
            self.audio2face_instance.make_avatar_speaks(audio_samples)   
